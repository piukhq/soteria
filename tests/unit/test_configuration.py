...
class TestConfiguration(TestCase):
    ...
    def test_configuration_init(self, mock_requests_retry_session, mock_vault):
    ...
    @mock.patch('soteria.configuration.hvac.Client')
    @mock.patch('soteria.configuration.requests_retry_session')
    def test_configuration_init_requests_exception_from_config_service(self, mock_requests_retry_session, mock_vault):
    ...
    @mock.patch('soteria.configuration.hvac.Client')
    @mock.patch('soteria.configuration.requests_retry_session')
    def test_configuration_init_bad_config_from_service(self, mock_requests_retry_session, mock_vault):
        mock_requests_retry_session.return_value.get.return_value.status_code = 200
        mock_requests_retry_session.return_value.get.return_value.json.return_value = {'error': 'Not found'}
        mock_vault.read.return_value = fixtures.MOCK_VAULT_RESPONSE
    ...
    @mock.patch('soteria.configuration.hvac.Client')
    @mock.patch('soteria.configuration.requests_retry_session')
    def test_configuration_init_vault_fail(self, mock_requests_retry_session, mock_vault):
        mock_requests_retry_session.return_value.get.return_value.status_code = 200
        mock_requests_retry_session.return_value.get.return_value.json.return_value = fixtures.MOCK_CONFIG_JSON
    ...
    @mock.patch('soteria.configuration.hvac.Client')
    @mock.patch.object(Configuration, '_get_config_data')
    def test_configuration_init_keys_not_saved_correctly(self, mock_get_config_data, mock_vault):
        mock_get_config_data.return_value = fixtures.MOCK_CONFIG_JSON
        mock_vault.return_value.read.return_value = ''
